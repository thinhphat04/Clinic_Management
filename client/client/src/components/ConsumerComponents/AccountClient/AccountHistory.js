...
const AccountHistory = () => {
  ...
  useEffect(() => {
    ...
    orders.map((order, index) => {
      const elementRowTable = document.querySelectorAll('.table__row-loading')[
        index
      ];
  ...
  const handleClickIsDelivering = (e) => {
    e.preventDefault();
    const btnFillter = document.querySelectorAll(
      '.account__box-info-fillter-btn',
    );
              ...
              <div className="account__box-info">
                ...
                <div className="account__box-info-counting">
                  <div className="account__box-info-counting-item">
                    ...
                    <div className="account__box-info-counting-title">
                      SẢN PHẨM ĐÃ MUA
                    </div>
                    <div className="account__box-info-counting-number">
                      {countOrder || 0} đơn hàng
                    </div>
                  </div>
                  ...
                  <div className="account__box-info-counting-item">
                    ...
                    <div className="account__box-info-counting-number">
                      {Number(countPriceOrder).toLocaleString() || 0} đ
                    </div>
                  </div>
                </div>
                ...
                <div className="account__box-info-fillter-container">
                  <div className="account__box-info-fillter">
                    <button
                      className="account__box-info-fillter-btn account__box-info-fillter-btn--active"
                      onClick={(e) => {
                        e.preventDefault();
                    ...
                    <button
                      className="account__box-info-fillter-btn"
                      onClick={handleClickIsDelivering}
                    >
                      Đang giao hàng
                    </button>
                    <button
                      className="account__box-info-fillter-btn"
                      onClick={handleClickDeliveried}
                    >
                      Đã giao hàng
                    </button>
                  </div>
                      ...
                      {loading ? (
                        ...
                        orders.map((order, index) => (
                          <tr className="table__row-loading" key={index}>
                            ...
                            <td style={{ color: 'red', textAlign: 'left' }}>
                              {order.lists.map((list, i) => (
                                <div className="table-td-element" key={i}>
                                  <img
                                    className="table-td__img"
                                    src={list.imageLink}
                                  ></img>
                                  <label
                                    style={{ color: 'var(--button-color)' }}
                                    className="table-td__label"
                                  >
                                    {list.productName}
                                  </label>
                                </div>
                            ...
                            >
                              {Number(order.price).toLocaleString() || 'None'} đ
                            </td>
                            <td
                            ...
                            <td>
                              <button
                                className="account-history__btn-detail"
                                onClick={(e) => {
                                  handleLoadingPage(1);
                                  setTimeout(() => {
